...
import com.example.permissionhelp.R;
import com.permission.sdk.listener.IPermissionCallback;
import com.permission.sdk.model.Constans;
import com.permission.sdk.model.PermissionOptions;
import com.permission.sdk.utils.Util;
	...
	public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults)
	{
		...
		if (requestCode == REQUEST_CODE)
		{
			...
			for (int i = 0, l = grantResults.length; i < l; i++)
			{
				...
				{
					allGranted = false;//拒绝权限

					if (mDeniedList == null)
						mDeniedList = new LinkedList<>();
					mDeniedList.add(permission);

					if (!ActivityCompat.shouldShowRequestPermissionRationale(this, permission))//用户点击了不再询问彻底拒绝了权限
						nerverAsk = true;//彻底拒绝权限设为true
				}
			}

			if (allGranted)
			{
				invokeAllGranted();//权限通过
			} else
			{
